...
public interface TransactionRepository extends JpaRepository<Transaction, Long>, JpaSpecificationExecutor<Transaction> {
    ...
    @Query("SELECT COALESCE(SUM(t.amount), 0) " +
           "FROM Transaction t " +
           "WHERE t.category.id = :categoryId AND t.category.type = com.finance.app.entity.CategoryType.EXPENSE AND t.transactionDate BETWEEN :startDate AND :endDate")
    ...
    @Query("SELECT t.category.id, COALESCE(SUM(t.amount), 0) " +
           "FROM Transaction t " +
           "WHERE t.category.type = com.finance.app.entity.CategoryType.EXPENSE AND t.transactionDate BETWEEN :startDate AND :endDate " +
           "GROUP BY t.category.id")
    List<Object[]> getTotalSpentGroupedByCategoryId(
            @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate);
}
